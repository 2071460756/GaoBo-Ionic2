...
var user={
    addUser:function (user,callback) {
        ...
        dom01.run(function () {
            that.getUserByPhoneNUm(user.userPhone,function (_res) {
                //判读是否已经存在改用户
                if(typeof _res=='object'){
                    if(_res.length!=0){
                        ...
                        getClient(function (client) {
                            ...
                            client.query(usesql.addUser,[user.userName,user.userPhone,util.MD5(user.password),user.token],function (error,result){
                                if(error){
                                    client.release();
                                    callback(3);
                                    return;
                                ...
                                // console.log(result);
                                console.log(result);
                                callback(result.affectedRows,user.userName,result.insertId);
                                client.release();
                            })
                        })
                    }
    ...
    inserentRelUser:function (user,callback) {
        dom01.on('error',function (err) {
            console.log(err.message);
            callback(3);
        });
        dom01.run(function () {
            getClient(function (client) {
                client.query(usesql.inserentRelUser,[user.relName,user.IDNumber,user.uId],function (error,result) {
                    if(error){
                        client.release();
                        //3  表示数据库连接异常
                        callback(3);
                ...
                client.query(usesql.getUserPassword,[user.phoneNum],function (error,result) {
                    ...
                    if(result.length==1){
                        ...
                        if(result[0].password==util.MD5(user.password)){
                            callback(1,result[0].userName,result[0].userId,result[0].photo);
    ...
    updatePhoto:function (users,callback) {
        ...
        dom01.run(function () {
            ...
            getClient(function (client) {
                client.query("UPDATE user_info SET photo =?  where userPhone = ?",[users.user_icon,users.phoneNum],function (error,result) {
                    if(error){
                        client.release();
                        //3  表示数据库连接异常
                        callback(3);
    ...
    addCollector:function (collInfo,callback) {
        ...
        dom01.run(function () {
            ...
            that.getCollById(collInfo.carId,collInfo.uId,function (_res) {
                if(typeof _res=='object'){
                    if(_res.length!=0){
                        //2 表示用户已存在
                        callback(2);
                        ...
                        getClient(function (client) {
                            client.query(usesql.insertCollector,[collInfo.carId,collInfo.uId],function (error,result) {
                                if(error){
                                    client.release();
                                    callback(3);
                                    console.log('插入错误')
    ...
    //判断收藏表中是否存在
    getCollById:function (carId,uId,callback) {
        dom01.on('error',function (err) {
            console.log(err.message);
            callback(3);
        });
        dom01.run(function () {
            getClient(function (client) {
                client.query(usesql.isExitCollById,[carId,uId],function (error,result) {
                    if(error){
                        client.release();
                        //3  表示数据库连接异常
                        callback(3);
    ...
    getPhoto:function (uId,callback) {
        ...
        dom01.run(function () {
            getClient(function (client) {
                client.query('select photo from user_info where userId=?',[uId],function (error,result) {
                    if(error){
                        client.release();
                        //3  表示数据库连接异常
                        callback(3);
    ...
    updateToken:function (token,userPhone,callback) {
        dom01.on('error',function (err) {
            console.log(err.message);
            callback(3);
        });
        dom01.run(function () {
            getClient(function (client) {
                client.query(usesql.updateToken,[token,userPhone],function (error,result) {
                    if(error){
                        client.release();
                        //3  表示数据库连接异常
                        callback(3);
