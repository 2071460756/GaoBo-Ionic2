...
var personal = {
    ...
    updateMyInfoByUid:function (upDateInfo,callback) {
        ...
        dom01.run(function (){
            getClient(function (client) {
                client.query(personalSql.upDateMyInfoByUid,[upDateInfo.myNickName,upDateInfo.myEmail,upDateInfo.MyUid],function (error,result) {
                    //更新出现错误
                    if (error) {
                        client.release();
                        //3  表示数据库连接异常
    ...
    //删除收藏的信息
    deleteByCarIdAndUid:function (uId,carId,callback) {
        dom01.on('error', function (err) {
            console.log(err.message);
            callback(3);
        });
        ...
        dom01.run(function () {
            getClient(function (client) {
                console.log(uId);
                console.log(carId);
                client.query(personalSql.deleteByCarIdAndUid,[uId,carId],function (error,result) {
                    if (error) {
                        console.log('出错了....');
                        client.release();
                        //3  表示数据库连接异常
