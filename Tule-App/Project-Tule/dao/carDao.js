...
var car={
    ...
    getTimeByCarId:function (carId,callback) {
        dom01.on('error',function (err) {
            console.log(err.message);
            callback(3);
        });
        ...
        dom01.run(function () {
            getClient(function (client) {
                client.query(carsql.getTime,[carId],function (error,result) {
                    if(error){
                        client.release();
                        //3  表示数据库连接异常
                        callback(3);
    ...
    getCounts:function (arr,callback) {
        dom01.on('error',function (err) {
            console.log(err.message);
            callback(3);
        });
        ...
        dom01.run(function () {
            getClient(function (client) {
                client.query(carsql.getCounts,[arr.store,arr.carId],function (error,result) {
                    if(error){
                        client.release();
                        //3  表示数据库连接异常
                        callback(3);
    ...
    getcarId:function (storm,callback) {
        dom01.on('error',function (err) {
            console.log(err.message);
            callback(3);
        });
        dom01.run(function () {
            getClient(function (client) {
                client.query(carsql.getcarId,[storm],function (error,result) {
                    if(error){
                        client.release();
                        //3  表示数据库连接异常
                        callback(3);
    ...
    getCarInfo:function (carId,callback) {
        ...
        dom01.run(function () {
            getClient(function (client) {
                client.query(carsql.getCarInfo,[carId],function (error,result) {
                    if(error){
                        client.release();
                        //3  表示数据库连接异常
                        callback(3);
