...
*/
    showRentALLInfo:'select * from allcar',
    showBrand:'select * from brand',
    showCarType:'select * from car_type',
    showCarCity:'select * from city',
    getCarStrom:'select * from car_store where cityId=?',
    showTime:'SELECT carId,rentStore,from_unixtime(unix_timestamp(rentTime),"%Y-%m-%d %h:00") as rentTime,  from_unixtime(unix_timestamp(returnTime),"%Y-%m-%d %h:00") as returnTime, orderStatusId from car_order',
