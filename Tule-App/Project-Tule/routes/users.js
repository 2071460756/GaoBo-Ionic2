...
router.post('/updateToken', function(req, res, next) {
    var user=req.body;
    console.log(user);
    userdao.updateToken(user.token,user.userPhone,function(result) {
...
router.post('/regist', function(req, res, next) {
    ...
    var user=req.body;
    if(user!=null && user.userPhone!=null && user.password!=null &&user.userName!=null){
        userdao.addUser(user,function(_result,name,uId) {
            res.json({result:_result,name:name,uId:uId});
        });
    }else {
        res.json({result:3});
    }
...
router.get('/getCollectorById', function(req, res, next) {
    var collInfo=req.query;
    if(collInfo!=null && collInfo.uId!=null && collInfo.carId!=null){
        userdao.getCollById(collInfo.carId,collInfo.uId,function(_result) {
...
router.get('/inserentRelUser', function(req, res, next) {
    var user=JSON.parse(req.query.user);
    userdao.inserentRelUser(user,function (result) {
        res.json(result.affectedRows);
    })

    // console.log(JSON.parse(user).relName);
    // userdao.getUidByPhoneNum(user.phoneNum,function (_result,result) {
    //     if(_result==1){
    //         res.json(result);
    //     }
